...
/* ===== VELOCITY CONTROL STATE (deg/s) ===== */
/* These are math-angle velocities set by the PC via 'V v1 v2'. */
static float g_vel_deg_per_sec_J1 = 0.0f;
static float g_vel_deg_per_sec_J2 = 0.0f;

/* Per-joint step timing for velocity stepping */
static unsigned long g_next_step_us_J1 = 0;
static unsigned long g_next_step_us_J2 = 0;
...
/* Global flags */
static bool g_homing_mode = false;   // during homing ignore soft band
...
bool tryStep(int i, bool dirCW){
  // Hard limit protection: limit is on CCW side for both joints
  if (limPressed(i)){
...
/* ===================== VELOCITY UPDATE (MATH DEG/S) ===================== */
/* ===================== VELOCITY UPDATE ===================== */
/* Called from loop() when no manual jog is active.
   Uses g_vel_deg_per_sec_J1/J2 (math deg/s) to generate steps.
*/
void vel_update(){
  unsigned long now = micros();

  // ---------- J1 ----------
  if (fabsf(g_vel_deg_per_sec_J1) > 1e-3f){
    float steps_per_sec = SPD_J1 * fabsf(g_vel_deg_per_sec_J1);
    if (steps_per_sec < 1.0f) steps_per_sec = 1.0f; // avoid huge periods
    unsigned long period = (unsigned long)(1000000.0f / steps_per_sec);

    if ((long)(now - g_next_step_us_J1) >= 0){
      bool dirCW = (g_vel_deg_per_sec_J1 < 0.0f);
      if (tryStep(0, dirCW)){
        g_next_step_us_J1 = now + period;
      } else {
        // blocked by limit/soft band -> stop this joint
        g_vel_deg_per_sec_J1 = 0.0f;
      }
    }
  } else {
    g_next_step_us_J1 = now;
  }

  // ---------- J2 ----------
  if (fabsf(g_vel_deg_per_sec_J2) > 1e-3f){
    float steps_per_sec = SPD_J2 * fabsf(g_vel_deg_per_sec_J2);
    if (steps_per_sec < 1.0f) steps_per_sec = 1.0f;
    unsigned long period = (unsigned long)(1000000.0f / steps_per_sec);

    if ((long)(now - g_next_step_us_J2) >= 0){
      bool dirCW = (g_vel_deg_per_sec_J2 > 0.0f);
      if (tryStep(1, dirCW)){
        g_next_step_us_J2 = now + period;
      } else {
        g_vel_deg_per_sec_J2 = 0.0f;
      }
    }
  } else {
    g_next_step_us_J2 = now;
  }
}
                          ...
                          float &x_closest, float &y_closest, float &z_closest)
{
  ...
  float A  = RX + L2 * c2;
  float psi = atan2f(yd, xd);
  float phi = atan2f(RY, A);
  float t1  = psi - phi;
...
bool home_one_J1(){
  ...
  g_homing_mode = true;
  g_vel_mode = false; g_vel_deg_per_sec[0]=0.0f; g_vel_deg_per_sec[1]=0.0f;
...
bool move_to_steps_coordinated(long tgtJ1, long tgtJ2){
  ...
  float rate1 = SPD_J1 * MAX_DEG_PER_SEC;
  float rate2 = SPD_J2 * MAX_DEG_PER_SEC;
...
bool move_to_raw_deg(float th1_raw_req, float th2_raw_req){
  float th1_m_req = J1_OFFSET_RAW_DEG - th1_raw_req;
  float th2_m_req = th2_raw_req + J2_MATH_MINUS_RAW;
  return move_to_math_deg(th1_m_req, th2_m_req);
}

/* ===================== IK ANALYZER ===================== */
struct IKReport {
  float th1_m_req, th2_m_req;
  float th1_m_cmd, th2_m_cmd;
      ...
      if (len>0){
        ...
        if (buf[0]=='Q' || buf[0]=='q'){
          ...
          if (sscanf(buf+1, "%f %f", &v1, &v2)==2){
            // Optional safety clamp to firmware max speed:
            if (v1 >  MAX_DEG_PER_SEC) v1 =  MAX_DEG_PER_SEC;
            if (v1 < -MAX_DEG_PER_SEC) v1 = -MAX_DEG_PER_SEC;
            if (v2 >  MAX_DEG_PER_SEC) v2 =  MAX_DEG_PER_SEC;
            if (v2 < -MAX_DEG_PER_SEC) v2 = -MAX_DEG_PER_SEC;

            g_vel_deg_per_sec_J1 = v1;
            g_vel_deg_per_sec_J2 = v2;
            // IMPORTANT: do NOT print anything here for high-rate control.
          } else {
            Serial.println("Usage: V v1_deg_per_s v2_deg_per_s");
          }
        } 
        else {
          char k = buf[0];
          if (k=='h'||k=='H'){ 
            e_stop=false; S[0].homed=S[1].homed=false; home_all(); 
          }
            ...
            Serial.println("Cmds: h | p | Q x y z | G x y z | M th1 th2 | R th1 th2 | V v1 v2 | 1/2 sel | a CCW | d CW | s stop");
          }
        }
        len=0;
      }
...
void loop(){
  ...
  // If manual jog is active, ignore velocity and use existing jog logic
  if (jog != JOG_NONE){
    int i = sel;
    ...
    // No manual jog: follow velocities set by 'V v1 v2'
    vel_update();
  }
}
