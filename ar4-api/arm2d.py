...
#         # ---- NEW: set joint velocities in math angle space (deg/s) ----
#     def set_velocity_math(self, v1_deg_s: float, v2_deg_s: float) -> MoveResult:
#         """
#         Set math-angle velocities for J1 and J2 (deg/s).

#         Uses new firmware command:
#             V <v1_deg_s> <v2_deg_s>

#         The Teensy will continuously step at these speeds in its main loop,
#         until you call this again (e.g. with zeros) or send a blocking move.

#         Returns:
#             MoveResult: ok=True if we got any response line mentioning [VEL],
#                         or if the write succeeded but no text came back.
#         """
#         lines = self.link.send_command(
#             f"V {v1_deg_s:.3f} {v2_deg_s:.3f}",
#             overall_timeout=0.3,  # We only expect a quick ack
#             quiet_gap=0.05,
#         )
#         ok = any("VEL" in ln.upper() for ln in lines) or (len(lines) == 0)
#         return MoveResult(ok=ok, reply=lines)
...
class Arm2D:
    ...
    #     Uses firmware command:
    #         V <v1_deg_s> <v2_deg_s>

    #     Teensy will keep stepping at these speeds in its loop() until
    #     you call this again or send a blocking move.
    #     """
    #     lines = self.link.send_command(
    #         f"V {v1_deg_s:.3f} {v2_deg_s:.3f}",
    #         overall_timeout=0.3,   # short: don't block the vision loop
    #         quiet_gap=0.05,
    #     )
    #     ok = any("VEL" in ln.upper() for ln in lines) or (len(lines) == 0)
    #     return MoveResult(ok=ok, reply=lines)

        # ---- NEW: continuous velocity control (math angles, deg/s) ----
    def set_velocity_math(self, j1_deg_per_s: float, j2_deg_per_s: float) -> None:
        """
        Set continuous math-angle velocities for J1 and J2 in deg/s.

        This is a fire-and-forget command: it does NOT wait for any reply
        from the Teensy, so it is safe to call from a high-rate vision loop
